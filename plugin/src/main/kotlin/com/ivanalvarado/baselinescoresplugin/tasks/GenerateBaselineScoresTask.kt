...
/**
 * Task for generating baseline scores for the project.
 *
...
abstract class GenerateBaselineScoresTask : DefaultTask(),
    ...
    fun execute() {
        try {
            val useCase = createGenerateBaselineScoresUseCase()
    ...
    private fun createGenerateBaselineScoresUseCase(): GenerateBaselineScoresUseCase {
        val serviceFactory = DefaultDomainServiceFactory()
        val baselineFileDetector = ConfigCacheCompatibleBaselineFileDetector()

        return GenerateBaselineScoresUseCase(
